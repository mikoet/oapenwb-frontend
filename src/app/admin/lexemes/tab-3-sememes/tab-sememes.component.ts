...
import { LemmaComponentComponent } from '../../../shared/_components/lemma-component.component'
...
@Component({
	...
	imports: [
		...
		MatHint,
		MatSlideToggle,
		DialectsSelectComponent_1,
		MatSelectTrigger,
		HierarchicalSelectComponent_1,
		LexemeLinkComponent,
		SynGroupLinkComponent_1,
...
export class TabSememesComponent implements OnInit, OnDestroy, SememeSupply {
	...
	set variantSupply(supply: VariantSupply) {
		...
		supply.addedEmitter.pipe(takeUntil(this.destroy$)).subscribe((variant) => this.variantAdded)
		supply.removedEmitter.pipe(takeUntil(this.destroy$)).subscribe((variant) => this.variantRemoved)
	}
	...
	ngOnInit(): void {
		this.data.loading.pipe(takeUntil(this.destroy$)).subscribe((isLoading) => {
			this.caseGovType = this.data.store.lexemeTypesByName.get('iCG')
		})

		this.sememeForm.valueChanges.pipe(takeUntil(this.destroy$)).subscribe(() => {
			this._errors = countErrors(this.sememeForm)
			if (this._trackChanges === true) {
				this._sememeChanged = true
			}
		...
		this.synGroupForm.valueChanges.pipe(takeUntil(this.destroy$)).subscribe(() => {
